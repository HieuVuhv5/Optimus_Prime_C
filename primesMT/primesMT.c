...
*set_prime_block(void *arg){
   ...
   // int arr_poss, numb_check, new_num = 0;
	...
	/* set for prime number 2 --> turn on bit at poss 2 to 1 */
	prime_arr[0].bits = (0x1 << 2 | prime_arr[0].bits);
	
	
	
	//printf("\n inside setprimeblock function: start_paws = %ld ub = %ld : numthread = %d", start_paws,ub, num_threads);
	for(long candy_date = start_paws; candy_date < ub; candy_date += (num_threads * 2)){
		for(long j = (candy_date); j <= (candy_date + num_threads); j += (candy_date)){
			int bb_index = j/32;
			int bb_bit = j%32;
			// do turn on  bit.
			uint32_t mask = 0x1 << bb_bit;
			//numb_check = bb_index * 32 + bb_bit;
			 //printf("candy :%ld j value: %ld\n",candy_date,j);
...
void op_Primes(void)
{
    ...
    //printf("\n Check output count : ");
	//ignore to check at bit 2 is number 2. 
	printf("2\n");
...
//    long tid = 0;
	...
	*/
	...
	/* print out all prime number from the bit_block bit */
	/* checked success */
